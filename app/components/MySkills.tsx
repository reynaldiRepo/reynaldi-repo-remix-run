...
function MainSkillCard(props: { MainSkill: MainSkillItem }) {
    return (
        <Box w={"300px"} maxW={"full"} minH={"276px"} bg={"yellow.400"} shadow={"base"} py={"30px"} px={"20px"} borderRadius={"base"}>
            <Center mb={"6"}>
                <Image
                    src={props.MainSkill.icon}
                    w={"60px"}
                />
            </Center>
            <Text textAlign={"center"} fontSize={"xl"} fontWeight={"semibold"} textTransform={"uppercase"} mb={"4"}>
    ...
    const mainSkillItems: MainSkillItem[] = [
        {
            ...
            description: "Analytical thinker, adept at breaking down complex problems for effective solutions.",
        },
        {
            icon: creativeIcon,
            title: "Creative Approach",
            description: "Fresh perspective, user-centric mindset for innovative solutions that captivate and deliver exceptional experiences.",
        },
        {
            icon: cleanCodeIcon,
            title: "Clean Code",
            description: "Prioritize maintainable, scalable code that is readable and efficient.",
        }
    ]
            ...
            <Box w={"full"} bg={"#F5F5F5"}>
                <Container maxW={"5xl"} py={isSmallerThan1020 ? "16" : "20"}>
                    <Box mb={"8"} display={"flex"} justifyContent={"center"}>
                        <Heading textTransform={"uppercase"} color={"yellow.400"}>Skills and Abilities</Heading>
                    </Box>
                    <Box display={"flex"}
                    ...
                    >
                        {mainSkillItems.map((skill, idx) => {
                            return (
                                <MainSkillCard MainSkill={skill} key={idx} />
                            )
                        })}
                    </Box>
                    <Box mt={10} mx={"auto"}
                        maxW={isSmallerThan975 ? "700px" : "900px"}
                    >
                        {hydrated ? 
                        <Slider
                            dots={false}
                            ...
                            {techSkills.map((t, idx) => {
                                return (
                                    <Tooltip label={t.text} fontSize='xs' key={idx}>
                                        <Box key={idx} display={"flex"} alignItems={"center"} h={"60px"} title={t.text} cursor={"grab"}>
